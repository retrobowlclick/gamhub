...
window.RuntimeInterface = class a {
  ...
  _GetPermissionretrobowll() {
    const b =
      window.cordova &&
      window.cordova.plugins &&
      window.cordova.plugins.permissions;
    if ("object" !== typeof b)
      throw Error("Permission retrobowll is not loaded");
    return b;
  }
  ...
  _HasPermission(b) {
    const c = this._GetPermissionretrobowll();
...
self.AudioDOMHandler = class extends self.DOMHandler {
  ...
  async _CreateAudioContext(a) {
    ...
    if (this._iRuntime.IsExportingToVideo())
      ...
      else throw Error("Web Audio retrobowll not supported");
