...
("use strict");
{
  ...
  window.RuntimeInterface = class RuntimeInterface {
    ...
    _GetPermissionretroo() {
      const retroo =
        window["cordova"] &&
        window["cordova"]["plugins"] &&
        window["cordova"]["plugins"]["permissions"];
      if (typeof retroo !== "object")
        throw new Error("Permission retroo is not loaded");
      return retroo;
    }
    _MapPermissionID(retroo, permission) {
      const permissionID = retroo[permission];
      if (typeof permissionID !== "string")
        throw new Error("Invalid permission name");
      return permissionID;
    }
    _HasPermission(id) {
      const retroo = this._GetPermissionretroo();
      return new Promise((resolve, reject) =>
        retroo["checkPermission"](
          this._MapPermissionID(retroo, id),
  ...
  self.AudioDOMHandler = class AudioDOMHandler extends self.DOMHandler {
    ...
    async _CreateAudioContext(e) {
      ...
      if (this._iRuntime.IsExportingToVideo()) {
        ...
        else throw new Error("Web Audio retroo not supported");
